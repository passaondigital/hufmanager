...
import { Building2, Navigation, AlertTriangle, Check, Loader2, MapPin } from "lucide-react";
...
export function CompanyLocationCard({
  ...
  const handleSave = async () => {
    ...
    if (error) {
      ...
      toast.success("Firmenstandort gespeichert!");
      setIsEditing(false);
      onUpdate();
    }
  };
            ...
            <div className="flex items-start gap-2 p-3 bg-warning/10 border border-warning/30 rounded-lg">
              ...
              <div className="text-sm">
                ...
                <p className="text-muted-foreground">
                  Für die Tour-Planung und Radius-Anzeige benötigen wir deinen Firmenstandort.
                </p>
              </div>
            </div>
          )}
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-base flex items-center gap-2">
          <Building2 className="h-4 w-4 text-primary" />
          Firmenstandort bearbeiten
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <p className="text-sm text-muted-foreground">
          Dieser Standort wird im Tour-Manager als Ausgangspunkt für die Radius-Anzeige verwendet.
        </p>
