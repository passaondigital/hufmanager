...
import { Users, Search, Ban, CheckCircle2, Clock, AlertTriangle } from "lucide-react";
...
export function AdminUserManagement() {
  ...
  const loadUsers = async () => {
    ...
    const { data, error } = await supabase
      .from("profiles")
      .select("id, email, full_name, first_name, last_name, role, created_at, avatar_url, account_status, trial_ends_at, last_active_at, phone")
    ...
    if (error) {
      ...
      setStats({
        total: data.length,
        providers: data.filter((u: any) => u.role === "provider").length,
        clients: data.filter((u: any) => u.role === "client").length,
        partners: data.filter((u: any) => u.role === "partner").length,
        newThisWeek: data.filter((u: any) => new Date(u.created_at) > weekAgo).length,
      });
    }
    setLoading(false);
  };
  ...
  const getName = (u: UserRow) => u.full_name || [u.first_name, u.last_name].filter(Boolean).join(" ") || "Kein Name";

  const getProfileCompleteness = (u: UserRow): number => {
    let score = 0;
    if (u.full_name || u.first_name) score += 25;
    if (u.email) score += 25;
  ...
  const getStatusBadge = (u: UserRow) => {
    ...
    const isSuspicious = completeness <= 25 && new Date(u.created_at) < new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
    ...
    return <Badge variant="outline" className="gap-1"><Clock className="h-3 w-3" />Trial {daysLeft != null && daysLeft > 0 ? `(${daysLeft}T)` : ""}</Badge>;
  };

  const filteredUsers = users.filter(u => {
    const name = getName(u);
    const matchesSearch = !search || name.toLowerCase().includes(search.toLowerCase()) || (u.email || "").toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
                ...
                <div className="flex items-start gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-medium text-sm truncate">{getName(u)}</p>
                    ...
                    <div className="flex items-center gap-3 mt-1 text-[10px] text-muted-foreground">
                      ...
                      <span>Profil: {getProfileCompleteness(u)}%</span>
                    </div>
                  </div>
                </div>
        ...
        {filteredUsers.length === 0 && <p className="text-center text-muted-foreground py-8">Keine Nutzer gefunden.</p>}
      </div>
    </div>
  );
}
