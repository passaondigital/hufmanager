...
-- Trigger function to notify users when features are unlocked/changed
...
BEGIN
  -- Only proceed if feature_statuses actually changed
  ...
  -- Find features that were unlocked (changed from disabled/beta to public/early_access, or from nothing to public)
  ...
  LOOP
    ...
    -- Feature was newly enabled or upgraded
  ...
  -- Only create notification if features were actually unlocked
...
DROP TRIGGER IF EXISTS trigger_notify_feature_change ON public.profiles;
CREATE TRIGGER trigger_notify_feature_change
  AFTER UPDATE OF feature_statuses ON public.profiles
  FOR EACH ROW
  EXECUTE FUNCTION public.notify_user_on_feature_change();
