-- Fix: Set all existing profiles to discoverable by default
UPDATE profiles SET is_discoverable = true WHERE is_discoverable = false;

-- Fix: Change default so new profiles are discoverable
ALTER TABLE profiles ALTER COLUMN is_discoverable SET DEFAULT true;

-- Fix search_profiles_universal: also filter out deleted profiles, 
-- and for email search use partial match
