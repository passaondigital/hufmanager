-- APPLY Migration #4 mit KANONISCHER Ledger-Version
...
--     kein Vault-Write, kein auto_invoice_enabled-Write
--
-- Aufruf:
--   psql "$DB_URL" -v ON_ERROR_STOP=1 -f docs/backups/mig4_20260917140000_apply_canonical.sql

begin;

create temp table _mig4 (t text) on commit drop;

insert into _mig4 (t) values ($MIG4TEXT$-- P1-2 correction (Codex Correction Pass 4 review of autoflow-auto-invoice,
...
-- DROP/CREATE TRIGGER needed, CREATE OR REPLACE FUNCTION above is enough.$MIG4TEXT$);

do $apply4$
declare s text; h text;
begin
  select t into s from _mig4;
  h := md5(s);
  if h <> '3056567c716fd7a318254042f6e878a4' then
...
select '20260917140000',
       'fix_autoflow_trigger_auth_vault_v1',
       ARRAY[t]::text[],
