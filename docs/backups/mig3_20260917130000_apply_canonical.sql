-- APPLY Migration #3 mit KANONISCHER Ledger-Version
...
--
-- Eigenschaften:
--   * eine einzige Transaktion: DDL + Ledger-Eintrag gemeinsam atomar
...
--   * md5-Guard bricht ab, falls der Text nicht dem Repo-Artefakt entspricht
--   * KEIN apply_migration -> keine serverseitig vergebene Version, kein Drift
--
-- Aufruf:
--   psql "$DB_URL" -v ON_ERROR_STOP=1 -f docs/backups/mig3_20260917130000_apply_canonical.sql

begin;

create temp table _mig3 (t text) on commit drop;

insert into _mig3 (t) values ($MIG3TEXT$-- P1-2 correction (Codex Correction Pass 4 review of autoflow-auto-invoice).
--
...
GRANT EXECUTE ON FUNCTION public.create_invoice_with_items_for_provider(uuid, uuid, jsonb, jsonb) TO service_role;$MIG3TEXT$);

do $apply3$
declare s text; h text;
begin
  select t into s from _mig3;
  h := md5(s);
  if h <> '427b0dfbfa98b518f92eaeacc6d3b086' then
...
select '20260917130000',
       'add_create_invoice_with_items_for_provider_v1',
       ARRAY[t]::text[],
