-- APPLY Migration #5 mit KANONISCHER Ledger-Version
...
--     erwartet: d0f8b9bd274a61b775f36c5e2711d45c
...
--     neue Objekte an und fasst keine Bestandsdaten an.
--   * #5 erteilt selbst KEINEN Kundenzugriff. Ihre einzige Wirkung ist, dass der
--     generische "erster Provider"-Fallback spaeter unterdrueckt werden KANN.
--     Solange #6-#9 nicht angewendet sind, ist sie inert.
--
-- Aufruf:
--   psql "$DB_URL" -v ON_ERROR_STOP=1 -f docs/backups/mig5_20260917150000_apply_canonical.sql

begin;

create temp table _mig5 (t text) on commit drop;

insert into _mig5 (t) values ($MIG5TEXT$-- FINAL ACCEPTANCE P0 (Tenant), Variante 3: serverseitige Grant-Unterdrückung
...
begin
  select t into s from _mig5;
  h := md5(s);
  if h <> 'd0f8b9bd274a61b775f36c5e2711d45c' then
...
select '20260917150000',
       'add_pending_client_invite_contract_v1',
       ARRAY[t]::text[],
